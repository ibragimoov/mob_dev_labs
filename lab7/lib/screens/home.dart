...
//A stream object that holds the duration of songs and the current position
...
//This widget indicates the permissionStatus,
  ...
  requestStoragePermission() async {
    ...
    if (!kIsWeb) {
      ...
      //If permissionsStatus is denied, then it will request. eg. The dialog box popped up when it is needed to use permission
      if (!permissionsStatus) {
        //If the permission is denied, then the request access will be displayed instead the list of songs
        setState(() {
  ...
  ConcatenatingAudioSource createPlaylist(List<SongModel>? songs) {
    List<ClippingAudioSource> sources = [];

    for (var song in songs!) {
      sources.add(
          ClippingAudioSource(child: AudioSource.uri(Uri.parse(song.uri!)), tag:
          MediaItem(
            id: '${song.id}',
            album: song.album,
            title: song.title,
